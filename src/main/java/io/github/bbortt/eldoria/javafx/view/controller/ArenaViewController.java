...
public class ArenaViewController implements Initializable {
    ...
    @FXML
    protected BorderPane viewBox;

    @FXML
    protected GridPane gameGrid;

    public ArenaViewController(Game game) {
        this.game = game;
    }

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        log.info("Starting tutorial arena");

        var board = game.getBoard().getTiles();

        for (var row = 0; row < board.length; row++) {
            for (var col = 0; col < board[row].length; col++) {
                var cell = new Rectangle(50, 50);
