import type { Move, PlayerID } from 'boardgame.io';
import { INVALID_MOVE } from 'boardgame.io/core';

import type { GameState } from '../game-state';
import { getPlayerString, logAction } from '../log';

const rollDice: Move<GameState> = ({ G, ctx, events, random }, playerID: PlayerID): typeof INVALID_MOVE | undefined => {
  ...
  logAction(`${getPlayerString(ctx.currentPlayer, playerID)} rolled ${diceRoll}.`);
  ...
  if (G.diceRoll['0'] !== 0 && G.diceRoll['1'] !== 0) {
    if (G.diceRoll['0'] === G.diceRoll['1']) {
      ...
      logAction(`${getPlayerString(startingPlayer, playerID)} rolled higher and will start the game.`);
    }
  }
